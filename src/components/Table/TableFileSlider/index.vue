	...
	<div class="con-table">
		...
		<div class="table-box-body" :style="{ minHeight: height - 36 + 'px' }">
			<table align="center" vertical="center" border="0" cellspacing="0" cellpadding="0">
				...
				<tbody>
					<tr v-for="(v, k) in dataList" :key="'list_' + k" @click="showOne(v)">
						<td :style="{ textAlign: t.align || 'left' }" v-for="(t, s) in columns" :key="'list_td_' + k + s">
							<p :class="[t.hasClassname ? v.classname : t.classname]">{{ v[t.dataIndex] }}</p>
						</td>
					</tr>
				</tbody>
			</table>
...
const showOne = (obj: any) => {
	let fileType = obj.wjlx;
	let fileUrl = obj.url;
	emit('showFile', fileType, fileUrl);
}
...
watch(() => props.dataList, (newValue: any) => {
	// 子项个数 * 40 < 总高度 = 停止滚动
	if ((newValue.length + 1) * 40 < props.height) {
