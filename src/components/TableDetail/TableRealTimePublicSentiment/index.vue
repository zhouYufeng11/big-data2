		...
		<div class="detail-filter">
			<a-form :model="form" auto-label-width>
				<a-space size="medium" align="baseline">
					...
					<a-form-item field="status" label="舆情来源">
					...
					<a-form-item field="content" label="舆情内容">
...
const initSelectDate = () => {
	const { startTime, endTime } = timeSlotChange(store.yqDate);
...
const fetchDicData = async () => {
	dic_interface2('type/jwpt_yqly').then((res: any) => {
		if (res.code === 200) {
			statusList.value = [
...
const columns: any = [
	...
	{
		title: '舆情内容', dataIndex: 'nr', ellipsis: true,
